...
import pprint
import scipy.misc
# import scipy.misc.imread as imread2
...
def imread(path):
    ...
    # return imread2(path).astype(np.float)
...
def center_crop_without_resize(x, crop_h, crop_w=None):
    if crop_w is None:
        crop_w = crop_h
    h, w = x.shape[:2]
    j = int(round((h - crop_h)/2.))
...
def random_crop(im, size):
    x = np.random.randint(im.shape[1] - size)
    y = np.random.randint(im.shape[0] - size)
...
def transform(image, npx=64, is_crop=True, resize_w=64):
    ...
    if is_crop:
        # cropped_image = center_crop_without_resize(image, npx)
